...
class Graph:
    ...
    def create_graph(self, column_ktype_mapping, *datasets):
        dataset_dfs = self._preprocess_data(datasets)
        ...
        for dataset in dataset_dfs:
            for column_name in dataset.columns:
                if column_name in column_ktype_mapping:
                    self.G.add_nodes_from(dataset[column_name], t=column_ktype_mapping[column_name])
    ...
    def _get_vertices_per_type(self):
        for type_i in range(self.types):
            ...
            nodes = [node_label for node_label, node_attr in self.G.nodes(data=True) if node_attr['t'] == type_i]
            ...
            for type_2 in range(type_1+1, self.types):
                ...
                for edge in subgraph.edges():
                    try:
                        ...
                        G_t_t_dash[type_1_vertex_position[edge[1]], type_2_vertex_position[edge[0]]] = 1
                
                self.Gtt[(type_1, type_2)] = G_t_t_dash
