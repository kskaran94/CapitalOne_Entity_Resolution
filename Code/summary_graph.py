...
class CMappings:
    ...
    def initialize_mapping(self, graph):
        number_super_nodes = self._calculate_initial_super_nodes(graph)
...
class SuperLink:
    ...
    def create_super_links(self, summary_graph):
        for type_i, type_i_supernodes in enumerate(summary_graph.S):
            ...
            for type_j in range(type_i+1, len(summary_graph.S)):
                type_j_supernodes = summary_graph.S[type_j]
...
class SummaryGraph:
    ...
    def create_summary_graph(self, graph, c_mapping):
        for type_i, type_i_c_mapping in enumerate(c_mapping.C):
            S_t = SuperNode()
